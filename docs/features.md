...
- **`kg_utils.temporal`** — Shared temporal contract for dated nodes: the `occurred_start` / `occurred_end` / `recorded_at` metadata keys, `temporal_metadata()` for producers, `read_span()` / `TemporalSpan` (`overlaps()`, `sort_key`) for consumers, and `spine_id()` / `spine_chain()` for deterministic calendar-node IDs. Precision is preserved — `"1876"` stays a year and overlaps any query touching 1876 — and *occurred* is kept distinct from *recorded*, so an entry written tonight about last Tuesday lands on Tuesday. Stdlib only
